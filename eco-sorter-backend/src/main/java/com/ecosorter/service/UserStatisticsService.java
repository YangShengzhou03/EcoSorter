...
public class UserStatisticsService {
    ...
    public UserStatisticsResponse getUserStatistics(Long userId) {
        ...
        Long actualClassificationCount = classificationRepository.selectCount(
            new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<Classification>()
                .eq(Classification::getUserId, userId)
        );
        
        UserStatisticsResponse response = new UserStatisticsResponse();
        response.setTotalCount(actualClassificationCount);
        ...
        if (actualClassificationCount > 0) {
            int accuracyRate = (int) ((statistics.getCorrectClassifications() * 100.0) / actualClassificationCount);
