...
#### 7.2 类图 🏷️
- **核心作用**：类图描述了系统的静态结构，定义了实体类、接口及其之间的关系。通过类图，可以深入了解系统内部的数据流动和逻辑关系。
- **关键类说明**：
| 类名 | 属性示例 | 方法示例 | 关系 |
| --- | --- | --- | --- |
...
| **AIProcessor** | modelVersion、accuracy | preprocessImage()、classify() | 实现→ImageRecognition接口 |
- **关系符号**：
  - **继承**（居民 → 系统用户）：使用`<<extends>>`符号表示，体现了子类继承父类的属性和方法。
  - **关联**（用户 → 积分记录）：用带箭头的实线表示，表明两个类之间存在关联关系。
  - **依赖**（AI处理器 → 数据库）：用带箭头的虚线表示，说明一个类的实现依赖于另一个类。
