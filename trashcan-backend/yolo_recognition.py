...
DATABASE_CONFIG = {
    'host': os.getenv('DB_HOST', 'ecosorter-mysql'),
...
async def register_face_with_file(userId: int, file: UploadFile = File(...)):
    try:
        print(f"[INFO] 开始人脸注册，用户ID: {userId}, 文件名: {file.filename}, 文件类型: {file.content_type}")
        contents = await file.read()
        print(f"[INFO] 接收文件大小: {len(contents)} bytes")
        
        if len(contents) > 512 * 1024:
            print(f"[WARN] 文件大小超过限制: {len(contents)} > 512KB")
        ...
        print(f"[INFO] 图片处理成功，形状: {image.shape}")
        
        print(f"[INFO] 开始检测人脸...")
        face_encodings = face_recognition.face_encodings(image)
        print(f"[INFO] 检测到人脸数量: {len(face_encodings)}")
        
        if len(face_encodings) == 0:
            print(f"[WARN] 未检测到人脸")
            return FaceRegisterResponse(success=False, message="无法检测到人脸，请确保照片中有清晰的正脸")
        
        print(f"[INFO] 人脸编码成功，编码长度: {len(face_encodings[0])}")
        encoding = json.dumps(face_encodings[0].tolist())
        
        print(f"[INFO] 开始更新数据库...")
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
        ...
        cursor.close()
        conn.close()
        print(f"[INFO] 数据库更新完成，影响行数: {affected_rows}")
        
        if affected_rows == 0:
            print(f"[WARN] 未找到用户ID: {userId}")
            return FaceRegisterResponse(success=False, message="用户不存在")
...
async def verify_face_with_file(file: UploadFile = File(...)):
    try:
        print(f"[INFO] 开始人脸验证，文件名: {file.filename}, 文件类型: {file.content_type}")
        contents = await file.read()
        print(f"[INFO] 接收文件大小: {len(contents)} bytes")
        ...
        cursor.close()
        conn.close()
        print(f"[INFO] 数据库中找到已注册人脸的用户数量: {len(users)}")
        
        if len(users) == 0:
            print(f"[WARN] 数据库中没有已注册人脸的用户")
            return FaceVerifyResponse(success=True, verified=False, message="未找到匹配的用户")
        ...
        for idx, user in enumerate(users):
            ...
            if stored_encoding:
                try:
                    ...
                    print(f"[DEBUG] [{idx+1}/{len(users)}] 用户 {username}(ID:{user_id}) 相似度: {similarity:.4f}, 距离: {distance:.4f}")
                    ...
                    print(f"[ERROR] 比对用户 {username} 时出错: {e}")
                    pass
        
        threshold = 0.7
        verified = best_score >= threshold
        
        print(f"[INFO] 比对完成 - 最佳匹配: {best_username}(ID:{best_user_id}), 相似度: {best_score:.4f}, 阈值: {threshold}, 验证结果: {verified}")
