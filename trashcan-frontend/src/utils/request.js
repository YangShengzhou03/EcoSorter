...
const javaApiBaseURL = process.env.VUE_APP_API_BASE_URL !== undefined ? process.env.VUE_APP_API_BASE_URL : 'http://localhost:8081'
const pythonApiBaseURL = process.env.VUE_APP_PYTHON_API_URL !== undefined ? process.env.VUE_APP_PYTHON_API_URL : 'http://localhost:9000'
...
const setupInterceptors = (axiosInstance) => {
  axiosInstance.interceptors.request.use(
    config => {
      ...
      let token = userToken || deviceToken
      
      if (config.url.includes('/api/trashcan/')) {
        token = deviceToken
      }
