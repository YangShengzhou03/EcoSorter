...
CREATE TABLE qr_login_sessions (
    ...
    qr_code VARCHAR(64) UNIQUE NOT NULL COMMENT '二维码ID',
    trashcan_id BIGINT NOT NULL COMMENT '垃圾桶ID',
    user_id BIGINT COMMENT '扫描用户ID',
    status VARCHAR(20) DEFAULT 'pending' COMMENT '状态: pending-待确认, confirmed-已确认, expired-已过期',
    scanned_at DATETIME COMMENT '扫描时间',
...
('unlogin_user', 'unlogin@qq.com', '123456', 'RESIDENT', true, NOW(), NOW());
...
(4, 0, 0, 0.00, 0.00, 0, 0, 0, 50, 200, NOW(), NOW());
